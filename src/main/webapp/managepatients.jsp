...
<title><%=L(hi,"मरीज़ प्रबंधन","Manage <%=L(hi,"मरीज़","Patient")%>s")%></title>
...
<div class="dash-main">
    ...
    <div class="page-hdr">
        <h2><i class="fa fa-users me-2" style="color:#19b37a"></i><%=L(hi,"मरीज़ प्रबंधन","Manage <%=L(hi,"मरीज़","Patient")%>s")%></h2>
        <p><%=L(hi,"देखें","View")%> and manage all registered patients.</p>
    </div>
    ...
    <div class="table-card">
        ...
        <table class="tbl">
            <thead><tr><th>#</th><th><%=L(hi,"मरीज़","Patient")%></th><th>Age / <%=L(hi,"लिंग","Gender")%></th><th><%=L(hi,"रक्त समूह","Blood Group")%></th><th>Contact</th><th><%=L(hi,"पता","Address")%></th><th><%=L(hi,"स्थिति","Status")%></th><th><%=L(hi,"कार्रवाई","Actions")%></th></tr></thead>
            <tbody>
            <%
            ...
            }else{ int idx=1; for(Object obj:patients){ <%=L(hi,"मरीज़","Patient")%> p=(<%=L(hi,"मरीज़","Patient")%>)obj; %>
            <tr>
                ...
                <td style="color:#374151"><%=p.getAge()%> / <%=p.get<%=L(hi,"लिंग","Gender")%>()!=null?p.get<%=L(hi,"लिंग","Gender")%>():"—"%></td>
                <td><span class="blood-badge"><%=p.getBloodGroup()!=null?p.getBloodGroup():"—"%></span></td>
                <td style="font-size:12px"><i class="fa fa-phone fa-xs me-1" style="color:#19b37a"></i><%=p.get<%=L(hi,"फ़ोन","Phone")%>()!=null?p.get<%=L(hi,"फ़ोन","Phone")%>():"—"%></td>
                <td style="font-size:12px;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><%=p.get<%=L(hi,"पता","Address")%>()!=null?p.get<%=L(hi,"पता","Address")%>():"—"%></td>
                <td><span class="badge badge-<%=p.get<%=L(hi,"स्थिति","Status")%>()%>"><%=p.get<%=L(hi,"स्थिति","Status")%>().toUpperCase()%></span></td>
                <td>
                    <div style="display:flex;gap:6px">
                        <a href="/HospitalManagement/admin/patients/view?id=<%=p.getId()%>" class="btn-view"><i class="fa fa-eye"></i> <%=L(hi,"देखें","View")%></a>
