...
.otp-sent-banner i{color:#065f46;font-size:18px}
.otp-sent-banner p{color:#065f46;font-size:13px;font-weight:600;margin:0}
.form-label{font-weight:600;font-size:13px;color:#374151;margin-bottom:6px;display:block;text-align:center}
...
function tick() {
    ...
    else { if(el) el.parentElement.innerHTML = '<span style="color:#ef4444;font-weight:600">OTP expired. Please request a new one.</span>'; }
}
window.onload = tick;
</script>
</head><body>
...
<div class="card">
    ...
    <% if(request.getAttribute("otpSent") != null || request.getAttribute("email") != null){ %>
    <div class="otp-sent-banner">
        <i class="fa fa-envelope-circle-check"></i>
        <p>OTP sent to <strong>${email}</strong>. Check your inbox!</p>
    </div>
    <% } %>
